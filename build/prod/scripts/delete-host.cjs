		...
		if ((e && typeof e == "object") || typeof e == "function")
			for (let n of Pe(e))
				!ke.call(i, n) &&
					...
					M(i, n, {
						get: () => e[n],
						enumerable: !(s = Ie(e, n)) || s.enumerable,
					});
		return i;
	};
var qe = (i) => Ne(M({}, "__esModule", { value: !0 }), i);
var pe = {};
Ae(pe, {
	AsyncAuthStore: () => Z,
	BaseAuthStore: () => C,
	BatchService: () => U,
	ClientResponseError: () => p,
	CollectionService: () => j,
	CrudService: () => R,
	HealthService: () => D,
	LocalAuthStore: () => q,
	...
	getTokenPayload: () => E,
	isTokenExpired: () => ee,
	normalizeUnknownQueryParams: () => F,
	serializeQueryParams: () => $,
});
function ue(i, e) {
	let t = {};
	if (typeof i != "string") return t;
	let s = Object.assign({}, e || {}).decode || _e,
		n = 0;
	for (; n < i.length; ) {
		let o = i.indexOf("=", n);
		if (o === -1) break;
		...
		if (r === -1) r = i.length;
			...
			n = i.lastIndexOf(";", o - 1) + 1;
			continue;
		}
		let a = i.slice(n, o).trim();
		if (t[a] === void 0) {
			let c = i.slice(o + 1, r).trim();
...
function B(i, e, t) {
	let s = Object.assign({}, t || {}),
		n = s.encode || je;
	if (!N.test(i)) throw new TypeError("argument name is invalid");
	let o = n(e);
...
function F(i) {
	if (i) {
		i.query = i.query || {};
		for (let e in i) De.includes(e) || ((i.query[e] = i[e]), delete i[e]);
	}
}
function $(i) {
	let e = [];
	for (let t in i) {
		let s = encodeURIComponent(t),
			n = Array.isArray(i[t]) ? i[t] : [i[t]];
		for (let o of n) ((o = Ue(o)), o !== null && e.push(s + "=" + o));
	}
	return e.join("&");
}
function Ue(i) {
	return i == null
		? null
		: i instanceof Date
			? encodeURIComponent(i.toISOString().replace("T", " "))
			: encodeURIComponent(typeof i == "object" ? JSON.stringify(i) : i);
}
function v(i, e, t, s) {
	let n = s !== void 0;
	return n || t !== void 0
...
function X(i) {
	return (
		i &&
		(i.constructor?.name === "FormData" ||
			(typeof FormData < "u" && i instanceof FormData))
	);
}
function le(i) {
	for (let e in i) {
		let t = Array.isArray(i[e]) ? i[e] : [i[e]];
		for (let s of t) if (Q(s)) return !0;
	}
	return !1;
}
function he(i) {
...
var p,
	...
	fe = Re(() => {
					...
					e !== null &&
						...
						e.response !== null && typeof e.response == "object"
							...
							: e.data !== null && typeof e.data == "object"
								...
								: (this.response = {})),
					this.originalError || e instanceof i || (this.originalError = e),
		...
		Le =
			...
			(typeof global < "u" && global.HermesInternal);
		de =
			typeof atob != "function" || Le
				? (i) => {
						let e = String(i).replace(/=+$/, "");
						if (e.length % 4 == 1)
							throw new Error(
		...
		((ae = "pb_auth"),
			(C = class {
				...
				exportToCookie(e, t = ae) {
					let s = { secure: !0, sameSite: !0, httpOnly: !0, path: "/" },
						n = E(this.token);
					((s.expires = n?.exp
						? new Date(1e3 * n.exp)
						: new Date("1970-01-01")),
					...
					let o = {
						...
						r = B(t, JSON.stringify(o), e),
						a = typeof Blob < "u" ? new Blob([r]).size : r.length;
					if (o.record && a > 4096) {
						o.record = { id: o.record?.id, email: o.record?.email };
						let c = ["collectionId", "collectionName", "verified"];
						for (let l in this.record)
							c.includes(l) && (o.record[l] = this.record[l]);
						r = B(t, JSON.stringify(o), e);
					}
					return r;
				}
			...
			(G = class extends O {
				...
				async testEmail(e, t, s, n) {
					return (
						(n = Object.assign(
							{
								method: "POST",
								body: { email: t, template: s, collection: e },
							},
							n,
						)),
						this.client.send("/api/settings/test/email", n).then(() => !0)
					);
				}
				async generateAppleClientSecret(e, t, s, n, o, r) {
					return (
						(r = Object.assign(
							{
								method: "POST",
		...
		((k = class extends O {
			...
			async subscribe(e, t, s) {
				...
				return (
					...
					this.isConnected
						...
						: await this.connect(),
					async () => this.unsubscribeByTopicAndListener(e, o)
				);
			}
			async unsubscribe(e) {
				if (e) {
					...
					for (let s in t)
						if (this.hasSubscriptionListeners(s)) {
							for (let n of this.subscriptions[s])
								this.eventSource?.removeEventListener(s, n);
			...
			async unsubscribeByTopicAndListener(e, t) {
				...
				for (let n in s) {
					if (
						...
						continue;
					let o = !1;
					for (let r = this.subscriptions[n].length - 1; r >= 0; r--)
						this.subscriptions[n][r] === t &&
							((o = !0),
							delete this.subscriptions[n][r],
							this.subscriptions[n].splice(r, 1),
							this.eventSource?.removeEventListener(n, t));
					o && (this.subscriptions[n].length || delete this.subscriptions[n]);
				}
				await this.submitSubscriptions();
			}
			hasSubscriptionListeners(e) {
				...
				for (let t in this.subscriptions)
					if (this.subscriptions[t]?.length) return !0;
				return !1;
			}
			...
			async finalizePendingSubscriptions() {
				...
				try {
					...
					for (let s of e) t ? s.reject(t) : s.resolve();
				} finally {
					((this.isProcessingPendingSubmits = !1),
						...
						((this.clientId = t?.lastEventId),
							...
							this.submitSubscriptions()
								.then(() => {
									...
									for (let n in s) for (let o of s[n]) o(e);
								})
								.catch((s) => {
									((this.clientId = ""),
										(this.lastSentSubscriptions = []),
										this.connectErrorHandler(s));
								}));
					}));
			}
			connectErrorHandler(e) {
				if (
					...
					for (let s of this.pendingConnects) s.reject(new p(e));
			...
			disconnect(e = !1) {
				if (
					...
					this.reconnectAttempts = 0;
					for (let t of this.pendingConnects) t.resolve();
					this.pendingConnects = [];
				}
			}
		}),
			(R = class extends O {
				...
				async getFullList(e, t) {
					...
					let s = 1e3;
					return (
						(t = Object.assign({}, e, t)).batch &&
							((s = t.batch), delete t.batch),
				...
				async getList(e = 1, t = 30, s) {
					return (
						...
						this.client
							...
							.then(
								(n) => (
									(n.items = n.items?.map((o) => this.decode(o)) || []),
									n
								),
							)
					);
				...
				_getFullList(e = 1e3, t) {
					...
					let s = [],
						n = async (o) =>
							this.getList(o, e || 1e3, t).then((r) => {
								let a = r.items;
								return (
									(s = s.concat(a)),
									a.length == r.perPage ? n(o + 1) : s
								);
							});
					return n(1);
				}
		...
		_ = class extends R {
			...
			async update(e, t, s) {
				return super.update(e, t, s).then((n) => {
					if (
						this.client.authStore.record?.id === n?.id &&
						(this.client.authStore.record?.collectionId ===
							this.collectionIdOrName ||
							this.client.authStore.record?.collectionName ===
								this.collectionIdOrName)
					) {
						let o = Object.assign({}, this.client.authStore.record.expand),
							r = Object.assign({}, this.client.authStore.record, n);
						(o && (r.expand = Object.assign(o, n.expand)),
			...
			async authWithPassword(e, t, s) {
				...
				((s = Object.assign(
					...
					this.isSuperusers &&
						((n = s.autoRefreshThreshold),
						delete s.autoRefreshThreshold,
						s.autoRefresh || W(this.client)));
				...
				return (
					...
					n &&
						...
						(function (a, c, l, u) {
							W(a);
							let d = a.beforeSend,
								y = a.authStore.record,
								f = a.authStore.onChange((g, b) => {
									(!g ||
										b?.id != y?.id ||
										((b?.collectionId || y?.collectionId) &&
											b?.collectionId != y?.collectionId)) &&
										W(a);
								});
							((a._resetAutoRefresh = function () {
								(f(), (a.beforeSend = d), delete a._resetAutoRefresh);
							}),
								(a.beforeSend = async (g, b) => {
									let re = a.authStore.token;
									if (b.query?.autoRefresh)
										return d ? d(g, b) : { url: g, sendOptions: b };
									let m = a.authStore.isValid;
									if (m && ee(a.authStore.token, c))
										try {
											await l();
										} catch {
											m = !1;
									...
									for (let w in I)
										if (
											w.toLowerCase() == "authorization" &&
											re == I[w] &&
											a.authStore.token
										) {
											I[w] = a.authStore.token;
											break;
										}
									return (
										(b.headers = I),
										d ? d(g, b) : { url: g, sendOptions: b }
									);
								}));
						})(
							this.client,
							...
							() =>
								this.authWithPassword(
									e,
									t,
									Object.assign({ autoRefresh: !0 }, s),
								),
						),
					o
				);
			}
			async authWithOAuth2Code(e, t, s, n, o, r, a) {
				let c = {
					method: "POST",
					body: {
						provider: e,
			...
			authWithOAuth2(...e) {
				...
				let t = e?.[0] || {},
					s = null;
				t.urlCallback || (s = ce(void 0));
				let n = new k(this.client);
				function o() {
				...
				return (
					...
					this.listAuthMethods(r)
						.then((c) => {
							let l = c.oauth2.providers.find((d) => d.name === t.provider);
							if (!l)
								throw new p(
									new Error(`Missing or invalid provider "${t.provider}".`),
								);
							let u = this.client.buildURL("/api/oauth2-redirect");
							return new Promise(async (d, y) => {
								let f = a ? this.client.cancelControllers?.[a] : void 0;
								(f &&
									(f.signal.onabort = () => {
										(o(),
											y(new p({ isAbort: !0, message: "manually cancelled" })));
									}),
									(n.onDisconnect = (g) => {
										g.length &&
											y &&
											(o(),
											y(new p(new Error("realtime connection interrupted"))));
									}));
								try {
									await n.subscribe("@oauth2", async (m) => {
										let I = n.clientId;
										try {
											if (!m.state || I !== m.state)
												throw new Error("State parameters don't match.");
											if (m.error || !m.code)
												throw new Error(
													"OAuth2 redirect error or missing code: " + m.error,
												);
											let w = Object.assign({}, t);
											(delete w.provider,
												delete w.scopes,
												delete w.createData,
												delete w.urlCallback,
												f?.signal?.onabort && (f.signal.onabort = null));
											let Te = await this.authWithOAuth2Code(
												l.name,
												m.code,
												l.codeVerifier,
												u,
									...
									let g = { state: n.clientId };
									t.scopes?.length && (g.scope = t.scopes.join(" "));
									let b = this._replaceQueryParams(l.authURL + u, g);
									await (
										t.urlCallback ||
										function (m) {
											s ? (s.location.href = m) : (s = ce(m));
										}
									)(b);
								} catch (g) {
									(f?.signal?.onabort && (f.signal.onabort = null),
			...
			async confirmPasswordReset(e, t, s, n, o) {
				let r = {
					method: "POST",
			...
			async confirmEmailChange(e, t, s, n) {
				let o = { method: "POST", body: { token: e, password: t } };
				return (
					(o = v(
					...
					this.client
						.send(this.baseCollectionPath + "/confirm-email-change", o)
						.then(() => {
							let r = E(e),
			...
			async impersonate(e, t, s) {
				(((s = Object.assign(
					{ method: "POST", body: { duration: t } },
					s,
				)).headers = s.headers || {}),
					s.headers.Authorization ||
						(s.headers.Authorization = this.client.authStore.token));
				let n = new V(this.client.baseURL, new C(), this.client.lang),
					o = await n.send(
						this.baseCollectionPath + "/impersonate/" + encodeURIComponent(e),
						s,
					);
				return (n.authStore.save(o?.token, this.decode(o?.record || {})), n);
			}
			_replaceQueryParams(e, t = {}) {
				...
				for (let a of r) {
					if (a == "") continue;
					let c = a.split("=");
					o[decodeURIComponent(c[0].replace(/\+/g, " "))] = decodeURIComponent(
						(c[1] || "").replace(/\+/g, " "),
					);
				}
				for (let a in t)
					t.hasOwnProperty(a) && (t[a] == null ? delete o[a] : (o[a] = t[a]));
				n = "";
				for (let a in o)
					o.hasOwnProperty(a) &&
						(n != "" && (n += "&"),
						(n +=
							...
							"=" +
							encodeURIComponent(o[a].replace(/%20/g, "+"))));
				return n != "" ? s + "?" + n : s;
			}
		};
		((j = class extends R {
			...
			(J = class extends O {
				...
				getURL(e, t, s = {}) {
					...
					let o = this.client.buildURL(n.join("/"));
				...
				async getToken(e) {
					return (
						...
						this.client.send("/api/files/token", e).then((t) => t?.token || "")
					);
				}
			}),
			(H = class extends O {
		...
		((U = class extends O {
			...
			collection(e) {
				return (
					this.subs[e] || (this.subs[e] = new x(this.requests, e)),
					this.subs[e]
				);
			}
			async send(e) {
				...
				for (let n = 0; n < this.requests.length; n++) {
					...
					if (
						...
						for (let r in o.files) {
							let a = o.files[r] || [];
							for (let c of a) t.append("requests." + n + "." + r, c);
						}
				}
				return (
			...
			(x = class {
				...
				prepareRequest(e, t) {
					...
					for (let n in s) {
						let o = s[n];
						if (Q(o)) ((e.files[n] = e.files[n] || []), e.files[n].push(o));
						else if (Array.isArray(o)) {
							let r = [],
								a = [];
							for (let c of o) Q(c) ? r.push(c) : a.push(c);
							if (r.length > 0 && r.length == o.length) {
								e.files[n] = e.files[n] || [];
								for (let c of r) e.files[n].push(c);
							} else if (((e.json[n] = a), r.length > 0)) {
								let c = n;
								(n.startsWith("+") || n.endsWith("+") || (c += "+"),
									(e.files[c] = e.files[c] || []));
			...
			(V = class {
					...
					((this.cancelControllers = {}),
						...
						t
							...
							: typeof window < "u" && window.Deno
								...
								: (this.authStore = new q()),
						(this.collections = new j(this)),
						(this.files = new J(this)),
						(this.logs = new L(this)),
						(this.settings = new G(this)),
						(this.realtime = new k(this)),
						(this.health = new D(this)),
						(this.backups = new H(this)),
						(this.crons = new Y(this)),
						(this.sql = new z(this)));
				}
				get admins() {
				...
				collection(e) {
					return (
						this.recordServices[e] || (this.recordServices[e] = new _(this, e)),
				...
				async send(e, t) {
					...
					if (this.beforeSend) {
						let n = Object.assign({}, await this.beforeSend(s, t));
					...
					return (
						...
						(t.fetch || fetch)(s, t)
							.then(async (n) => {
								...
								if (
									(this.afterSend && (o = await this.afterSend(n, o, t)),
									n.status >= 400)
								)
									throw new p({ url: n.url, status: n.status, data: o });
								return o;
							})
							.catch((n) => {
								throw new p(n);
				...
				initSendOptions(e, t) {
					if (
						(((t = Object.assign({ method: "GET" }, t)).body = (function (n) {
							if (
								typeof FormData > "u" ||
								n === void 0 ||
								typeof n != "object" ||
							...
							for (let r in n) {
								let a = n[r];
								if (a !== void 0)
									if (typeof a != "object" || le({ data: a })) {
										let c = Array.isArray(a) ? a : [a];
										for (let l of c) o.append(r, l);
									} else {
										let c = {};
										((c[r] = a), o.append("@jsonPayload", JSON.stringify(c)));
									}
							}
							return o;
						})(t.body)),
			...
			(Z = class extends C {
				constructor(e) {
					(super(),
						(this.queue = []),
						(this.saveFunc = e.save),
						(this.clearFunc = e.clear),
						this._enqueue(() => this._loadInitial(e.initial)));
				}
				save(e, t) {
					super.save(e, t);
					let s = "";
					try {
						s = JSON.stringify({ token: e, record: t });
					} catch {
					...
					try {
						if ((e = await e)) {
							...
							(typeof e == "string"
								? (t = JSON.parse(e) || {})
								: typeof e == "object" && (t = e),
								this.save(t.token || "", t.record || t.model || null));
						}
					} catch {}
				}
				_enqueue(e) {
...
var me = oe((nt, be) => {
	var ge = (fe(), qe(pe)),
		Ve = ge.default || ge,
		{ execSync: T } = require("child_process"),
		te = class extends Ve {
			constructor(e) {
				super(e);
			}
			async initAuth(e, t) {
				await this.collection("_superusers").authWithPassword(e, t);
			}
			async getAll(e) {
				if (!e) throw new Error("A collection must be specified for getAll");
				return await this.collection(e).getFullList();
			}
			async getOne(e, t) {
				if (!e) throw new Error("A collection must be specified for getOne");
				if (!t) throw new Error("A filter must be specified for getOne");
				let s = await this.collection(e).getList(1, 1, { filter: t });
				return s.items.length === 0 ? null : s.items[0];
			}
			async postOne(e, t) {
				if (!e) throw new Error("A collection must be specified for postOne");
				if (!t) throw new Error("Data must be specified for postOne");
				await this.collection(e).create(t);
			}
			async deleteOne(e, t) {
				await this.collection(e).delete(t);
			}
			async deleteHostMetadata(e) {
				(T(`sudo a2dissite ${e.repo} ${e.repo}-le-ssl`),
					T(
						`sudo rm /etc/apache2/sites-available/${e.repo}.conf /etc/apache2/sites-available/${e.repo}-le-ssl.conf`,
					),
					T(`docker stop ${e.name}-container`),
					T(`docker rm ${e.name}-container`),
					T(`docker rmi ${e.name}-image`),
					T(`certbot delete --cert-name ${e.name}.is-prod.applications.ws`, {
						stdio: ["pipe", "inherit", "inherit"],
						input: `y
`,
					}),
					T("apache2ctl restart"));
			}
		};
	be.exports = te;
});
var Ee = oe((it, S) => {
	...
	function Ke() {
		return ye[Math.floor(Math.random() * ye.length)];
	}
	function P(i) {
		return typeof i == "string"
			? !["false", "0", "no", "off", ""].includes(i.toLowerCase())
			: !!i;
	}
	function Me() {
...
`,
	...
	function Je(i) {
		...
		for (let r = 0; r < n; r++)
			try {
				let a = s[r].trim(),
					c = Ye(t, a);
				o = h.decrypt(c.ciphertext, c.key);
				break;
			} catch (a) {
				if (r + 1 >= n) throw a;
			}
	...
	function ve(i) {
		...
		if (i && i.path && i.path.length > 0)
			if (Array.isArray(i.path))
				for (let t of i.path)
					se.existsSync(t) && (e = t.endsWith(".vault") ? t : `${t}.vault`);
			else e = i.path.endsWith(".vault") ? i.path : `${i.path}.vault`;
		else e = K.resolve(process.cwd(), ".env.vault");
		return se.existsSync(e) ? e : null;
	}
	function we(i) {
		return i[0] === "~" ? K.join(Fe.homedir(), i.slice(1)) : i;
	}
	function ze(i) {
		let e = P(process.env.DOTENV_CONFIG_DEBUG || (i && i.debug)),
			t = P(process.env.DOTENV_CONFIG_QUIET || (i && i.quiet));
		(e || !t) && Oe("loading env from encrypted .env.vault");
		let s = h._parseVault(i),
			n = process.env;
		return (
			i && i.processEnv != null && (n = i.processEnv),
			h.populate(n, s, i),
			{ parsed: s }
		);
	}
	function Qe(i) {
		let e = K.resolve(process.cwd(), ".env"),
			t = "utf8",
			s = process.env;
		i && i.processEnv != null && (s = i.processEnv);
		let n = P(s.DOTENV_CONFIG_DEBUG || (i && i.debug)),
			o = P(s.DOTENV_CONFIG_QUIET || (i && i.quiet));
		i && i.encoding
			? (t = i.encoding)
			: n && A("no encoding is specified (UTF-8 is used by default)");
		let r = [e];
		if (i && i.path)
			if (!Array.isArray(i.path)) r = [we(i.path)];
			else {
				r = [];
				for (let u of i.path) r.push(we(u));
			}
		let a,
			c = {};
		for (let u of r)
			try {
				let d = h.parse(se.readFileSync(u, { encoding: t }));
				h.populate(c, d, i);
			} catch (d) {
				(n && A(`failed to load ${u} ${d.message}`), (a = d));
			}
		let l = h.populate(s, c, i);
		if (
			((n = P(s.DOTENV_CONFIG_DEBUG || n)),
			(o = P(s.DOTENV_CONFIG_QUIET || o)),
			n || !o)
		) {
			...
			for (let y of r)
				try {
					let f = K.relative(process.cwd(), y);
					d.push(f);
				} catch (f) {
					(n && A(`failed to load ${y} ${f.message}`), (a = f));
				}
			Oe(`injected env (${u}) from ${d.join(",")} ${We(`// tip: ${Ke()}`)}`);
		}
		return a ? { parsed: c, error: a } : { parsed: c };
	}
	function Xe(i) {
		if (Se(i).length === 0) return h.configDotenv(i);
		let e = ve(i);
		return e
			? h._configVault(i)
			: (He(`you set DOTENV_KEY but you are missing a .env.vault file at ${e}`),
				h.configDotenv(i));
	}
	function Ze(i, e) {
		let t = Buffer.from(e.slice(-64), "hex"),
			s = Buffer.from(i, "base64"),
			n = s.subarray(0, 12),
			o = s.subarray(-16);
		s = s.subarray(12, -16);
		try {
			let r = $e.createDecipheriv("aes-256-gcm", t, n);
			return (r.setAuthTag(o), `${r.update(s)}${r.final()}`);
		} catch (r) {
			let a = r instanceof RangeError,
				c = r.message === "Invalid key length",
				l = r.message === "Unsupported state or unable to authenticate data";
			if (a || c) {
				let u = new Error(
	...
	S.exports.configDotenv = h.configDotenv;
	S.exports._configVault = h._configVault;
	S.exports._parseVault = h._parseVault;
	S.exports.config = h.config;
	S.exports.decrypt = h.decrypt;
	S.exports.parse = h.parse;
	S.exports.populate = h.populate;
	S.exports = h;
});
var tt = me(),
	Ce = Ee(),
	ne = Ce.config({ path: "/root/scripts/.env" });
ne.error && (ne = Ce.config({ path: "./scripts/.env" }));
if (ne.error) throw new Error("No .env file found.");
...
var ie = process.argv[2];
if (!ie)
	throw new Error(
		"A repo name must be provided to the script in order to delete a host.",
	);
(async () => {
	let i = new tt("http://127.0.0.1:8090");
	try {
		await i.health.check();
	} catch {
		throw new Error(
			"No connection to PocketBase on localhost:8090, are you running an instance of PocketBase locally?",
		);
	}
	await i.initAuth(process.env.EMAIL, process.env.PASSWORD);
	let e = await i.getOne("hosts", `repo="${ie}"`);
	if (!e) throw new Error(`Repo name ${ie} was not found!`);
	(process.env.ENVIRONMENT === "dev"
		? console.log(
				"Deleting host metadata is not possible in a local dev environment.",
			)
		: await i.deleteHostMetadata(e),
		await i.deleteOne("hosts", e.id),
		process.exit(0));
})().catch((i) => {
